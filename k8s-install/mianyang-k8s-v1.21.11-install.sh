...
EOF
    ...
    # 仅 enable kubelet，不 start（此时无 kubeconfig，启动必然失败）
...
pull_images() {
    ...
    for aliyun_image in "${!IMAGES[@]}"; do
        current=$((current + 1))
        target_image="${IMAGES[$aliyun_image]}"
...
# ==================== 显示帮助信息 ====================
show_help() {
    echo -e "${CYAN}Kubernetes 节点安装初始化脚本 v2${NC}
...
${YELLOW}功能说明:${NC}
  此脚本用于在 Ubuntu 22.04 上安装 Kubernetes 节点，
  包括 Docker、kubelet、kubeadm、kubectl 的安装和配置。
