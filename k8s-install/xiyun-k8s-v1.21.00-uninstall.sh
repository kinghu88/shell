...
LOG_FILE="${LOG_DIR}/uninstall-xiyun-$(date '+%Y%m%d-%H%M%S').log"

IMAGES=(
    "prom/node-exporter:v1.3.1"
    "kubesphere/kube-rbac-proxy:v0.11.0"
    "calico/cni:v${CALICO_VERSION}"
    "calico/pod2daemon-flexvol:v${CALICO_VERSION}"
...
remove_packages() {
    ...
    log_info "取消版本锁定..."
    apt-mark unhold kubelet kubeadm kubectl docker.io >> "${LOG_FILE}" 2>&1 || true
    ...
    log_info "卸载 Docker..."
    if apt-get purge -y docker.io docker-doc docker-compose docker-compose-v2 podman-docker containerd runc 2>&1 | tee -a "${LOG_FILE}"; then
...
remove_sources() {
    ...
    rm -f /etc/apt/keyrings/kubernetes-huawei.gpg 2>/dev/null || true
...
# ==================== 帮助信息 ====================
show_help() {
    echo -e "${CYAN}Kubernetes 节点卸载脚本 v2（希云环境）${NC}
...
${YELLOW}默认行为:${NC}
  ...
  • 卸载 kubelet / kubeadm / kubectl / docker.io / containerd
  • 清理 APT 源与 docker 配置
...
main() {
    ...
    mkdir -p "${LOG_DIR}"
    echo "===== Kubernetes 卸载日志（希云环境）$(date) =====" > "${LOG_FILE}"
    ...
    print_separator
    log_info "开始卸载 Kubernetes 节点（希云环境）..."
    log_info "Kubernetes 版本: ${K8S_VERSION}"
    log_info "Docker 版本: ${DOCKER_VERSION}"
    log_info "日志文件: ${LOG_FILE}"
    print_separator

    # --purge 时先清理镜像（在卸载 Docker 之前）
    if $purge; then
        purge_data
    fi
